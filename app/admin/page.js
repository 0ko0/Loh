...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState('custom_home'); // 'scripts' | 'user_logs' | 'custom_home' | 'settings'
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] bg-green-500/15 text-green-400 border border-green-500/30 px-2 py-0.5 rounded-full font-bold flex items-center gap-1">
                <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-ping" /> ONLINE
              </span>
            </div>
            <p className="text-[11px] text-gray-400 font-medium">Bảng điều khiển Raw Script & Cấu hình Website</p>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              <h2 className="text-sm font-black text-[#6E96FF] flex items-center gap-2 uppercase tracking-wide">
                <Users className="w-4 h-4" /> Lịch Sử Log Người Dùng
              </h2>
            </div>
